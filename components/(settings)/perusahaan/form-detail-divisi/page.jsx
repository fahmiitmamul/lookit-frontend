...
const DetailDivisionForm = ({ showViewDivisionModal }) => {
    ...
    const postDivision = useMutation({
        mutationFn: async (values) => {
            ...
            return http(token).post('/division', data)
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['division'] })
            dispatch(setLoading(false))
            showViewDivisionModal(false)
            toast.success('Berhasil menambah divisi')
        },
        onError: (err) => {
...
export default DetailDivisionForm
