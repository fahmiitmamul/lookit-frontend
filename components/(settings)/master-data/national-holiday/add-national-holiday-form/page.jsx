...
const AddNationalHolidayForm = ({ showAddNationalHolidayModal }) => {
    ...
    const postNationalHoliday = useMutation({
        mutationFn: async (values) => {
            const data = new URLSearchParams(values).toString()
            return http(token).post(`/national-holiday`, data)
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['national-holiday'] })
            dispatch(setLoading(false))
            showAddNationalHolidayModal(false)
            toast.success('Berhasil menambah libur nasional')
        },
        onError: (err) => {
    ...
    const onSubmit = (data) => {
        ...
        postNationalHoliday.mutate(data)
    }
...
export default AddNationalHolidayForm
