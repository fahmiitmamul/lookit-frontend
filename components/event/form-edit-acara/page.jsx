...
const EditEventsForm = ({ setShowEditEventsModal, eventId }) => {
    ...
    const patchEvents = useMutation({
        ...
        onSuccess: () => {
            ...
            toast.success('Berhasil mengedit acara')
        },
        onError: (err) => {
    ...
    const onSubmit = (data) => {
        ...
        patchEvents.mutate(data)
        dispatch(setLoading(true))
    }

    const handleDelete = useMutation({
        mutationFn: async (id) => {
            return http(token).delete(`/events/${eventId}`)
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['events'] })
